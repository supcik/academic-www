...
Currently, the widget is in *Beta* status. To check out the People widget,

1. Add the `user_groups` option for each user as per the the [demo user](https://raw.githubusercontent.com/gcushen/hugo-academic/master/exampleSite/content/author/admin/_index.md)
...
4. Edit the People widget's `user_groups` to determine which groups to display
